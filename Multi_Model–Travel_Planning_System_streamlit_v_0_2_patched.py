...
class HotelRecommenderAgent:
    ...
    def add_hotels(self, hotels):
        ...
        self.hotels_embeddings = np.asarray(self.hotels_embeddings, dtype='float32')

    def find_hotels(self, preferences, top_k=3):
        pref_embedding = self.encoder.encode([preferences])
        pref_embedding = np.asarray(pref_embedding, dtype='float32')
...
st.caption(f"Encoder device: {hotel_agent.device}")
